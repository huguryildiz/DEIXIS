...
import { ChevronRight, FlaskConical, Menu, Moon, Plus, Settings2, Sun } from 'lucide-react'
...
type Route = { view: 'home' } | { view: 'research'; id: string } | { view: 'connections' }
type ThemeChoice = 'system' | 'light' | 'dark'

function parseHash(): Route {
  const hash = window.location.hash.replace(/^#\/?/, '')
  if (hash.startsWith('research/')) return { view: 'research', id: hash.slice('research/'.length) }
  if (hash === 'connections') return { view: 'connections' }
  return { view: 'home' }
}

export function go(route: Route) {
  window.location.hash = route.view === 'research' ? `/research/${route.id}` : route.view === 'connections' ? '/connections' : '/'
}
  ...
  return <div className={`app ${dark ? 'dark' : ''}`}>
    ...
    <div className="main-shell">
      <header>
        ...
        <div className="header-actions"><Button variant="ghost" size="icon" onClick={toggleTheme} aria-label={dark ? 'Use light theme' : 'Use dark theme'}>{dark ? <Sun size={17} /> : <Moon size={17} />}</Button></div>
      </header>
      <main>
        {route.view === 'home' && <Home researches={researches} onCreated={id => { refreshList(); go({ view: 'research', id }) }} />}
        {route.view === 'research' && <ResearchPage key={route.id} id={route.id} dark={dark} onChanged={refreshList} />}
