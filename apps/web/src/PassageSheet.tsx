    ...
    <SheetContent className={`detail-sheet source-sheet ${full ? 'is-full' : ''} ${dark ? 'dark' : ''}`}>
      ...
      <div className="sheet-body">
        ...
        {(passage || assetText || detailsOnly) && source && <>
          ...
          <div className="tab-strip source-view-tabs" role="tablist" aria-label={t('Source view')}>
            ...
            {pdfAssetId ? <button type="button" role="tab" aria-selected={viewMode === 'pdf'} aria-controls="source-pdf-view" onClick={() => { setViewMode('pdf'); setFull(true) }}>PDF</button>
              : <Tooltip content={t(pdfUnavailable)}><button type="button" role="tab" aria-selected={false} aria-disabled="true" aria-describedby="source-pdf-unavailable">PDF</button></Tooltip>}
            {!pdfAssetId && <span id="source-pdf-unavailable" className="sr-only">{t(pdfUnavailable)}</span>}
          </div>
