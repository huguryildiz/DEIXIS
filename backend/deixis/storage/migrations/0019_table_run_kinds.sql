...
-- Evidence table work (P5) runs through the same run, step and StepInput records. SQLite cannot widen a CHECK in place,
-- and six tables reference runs, so the table is rebuilt with foreign keys off; migrate() checks them before the commit.
CREATE TABLE runs_new (
  ...
  -- Table runs: {"table_id", "column_ids", "cell_id", "include_stale"}. NULL for discovery and answer runs.
  target_json TEXT,
