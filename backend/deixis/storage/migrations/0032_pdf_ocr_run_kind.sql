...
-- Scanned PDF pages are read with local OCR in a run of their own (D51). SQLite cannot widen a CHECK in place, so the
-- table is rebuilt as in 0028; migrate() checks foreign keys.
CREATE TABLE runs_new (
  id TEXT PRIMARY KEY,
  research_id TEXT NOT NULL REFERENCES researches(id),
  scope_revision INTEGER NOT NULL,
  kind TEXT NOT NULL CHECK (kind IN ('discovery', 'answer', 'table_columns', 'table_fill', 'cell_recheck', 'research_title', 'pdf_collection', 'pdf_ocr')),
