...
-- The `fulltext_adjudication` run kind follows a completed `sw` full-text retrieval run and reads the works whose
-- text arrived (D85, slice 12). SQLite cannot widen a CHECK in place, so runs is rebuilt as in 0045; migrate()
-- checks foreign keys after commit.
CREATE TABLE runs_new (
  id TEXT PRIMARY KEY,
  research_id TEXT NOT NULL REFERENCES researches(id),
  scope_revision INTEGER NOT NULL,
  kind TEXT NOT NULL CHECK (kind IN ('discovery', 'answer', 'table_columns', 'table_fill', 'cell_recheck', 'research_title', 'pdf_collection', 'pdf_ocr', 'report', 'fulltext_fetch', 'fulltext_adjudication')),
