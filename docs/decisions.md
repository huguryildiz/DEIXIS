...
**Decision:** Before its first provider request a discovery run freezes one protocol record (`protocol_records`, migration 0037) through a `protocol` step, so a paused and resumed run keeps the record it froze. The body is built by the pure `workflow/protocol.py::build_protocol` and holds the question and scope, the vocabulary the plan proposed, the compiled provider queries, the providers, arms, thresholds read from their one definition, the model and effort per step role, the method package hash, the code version and the query strategy. A record is never edited in place: a changed body opens the next protocol revision and needs its reason, and permanent deletion of the research is the only authorized delete, as it is for step inputs. Every step row opened after the freeze carries `protocol_hash`. Audit digests are taken over one canonical form (`domain/canonical.py`: sorted keys, NFC text, sets and tuples as sorted lists, rows sorted by a stable identifier), never over raw bytes: `step_inputs.payload_sha256`, `model_sessions.output_sha256` (prefixed `json:` or `text:` by what was read) and `search_runs.payload_sha256`. Orderings that feed a decision break ties by a stable identifier: `fuse_rankings` on the record id, `answer_source_order` on the source id after selection order, `store.included_sources` and the FTS passage query by identifier as their last key. `Settings.search_workflow` (`DEIXIS_SEARCH_WORKFLOW`, `legacy` by default) is stored on the scope revision and written into the protocol; nothing branches on it yet. `tests/test_determinism.py` replays five code stages in separate processes under two hash seeds and two row orders and requires one canonical digest.

**Limits:** The protocol body's criterion, criterion parts, cue phrases, concept blocks, claim words, exclusion words and signal list are written as null and are filled by later slices; the reviewer that follows the app-wide default setting is not resolved into the body, because the body is built without a store. A research whose scope is `attached` runs no discovery, so it has no protocol record and its answer run's steps carry no hash. `code_version` is the package version and the query-compiler version text, so it does not change per commit and meets SW14 point 5's "same code version" weakly. No embedding was recomputed, so floating-point differences of an embedding model across machines were not seen. The SQLite FTS tie order was fixed with a second sort key only; it was not measured. The tests run on SYNTHETIC fixtures and a scripted model: they show workflow behavior, not model quality or live provider access.
