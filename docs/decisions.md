...
**Status**: accepted (sub-steps 1–2 implemented: finding image and blank pages, reading one page with Tesseract, merging OCR text; storing it with `ocr_json` in migration 0031, `passages.text_source` shared with D52's 0030, and an `asset_ocr_read` event. Sub-steps 3–6 not implemented)
**Date**: 2026-09-16
...
**Limits**: There is no `pdf_ocr` run, endpoint, label or warning yet. An OCR reading is recorded under its version name, so a reading that found no text or failed on some pages cannot be repeated with the same Tesseract version and languages until sub-step 3 decides how. Timings in the design note come from four real scans on one machine. PyMuPDF's OCR gives no word confidence, so there is no quality threshold beyond "text found". OCR text of equations is expected to be wrong in places; the label and warning say so but do not detect it.
