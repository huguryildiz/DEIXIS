...
**Status**: accepted (sub-steps 1–4 implemented: removing and restoring a source; the trash of tables, templates and columns; restoring a wrong-file PDF. Undo notifications, a table from selected sources, backup checks and the interface are not)
**Date**: 2026-09-16
...
**Limits**: Only sub-steps 1–4 are implemented. Undo notifications, a table from selected sources, backup checks of removed memberships and the interface are not. Deleting a work or file from the whole library, trash expiry, undoing a cell revision, notes, starting a table from the Library and OCR are outside this decision. A permanent table purge cannot be undone except from a backup.
