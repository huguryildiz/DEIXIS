...
**Limits**: There is no OCR label, model input field, warning or screen yet; a `pdf_ocr` run is not shown in the research transcript. A page that fails pauses the run before anything is written, and resuming reads only the failed pages again; a reading that was written, including one rejected because it found no text, is not repeated with the same Tesseract version and languages (the endpoint answers 409). Timings in the design note come from four real scans on one machine. PyMuPDF's OCR gives no word confidence, so there is no quality threshold beyond "text found". OCR text of equations is expected to be wrong in places; the label and warning say so but do not detect it.
