...
**Tarih:** 16 Eylül 2026. **Durum:** §9'daki sorular sahibin isteğiyle Claude tarafından yanıtlandı ([D37](../decisions.md)). Modelsiz ilk alt adım uygulandı: `0019`/`0020` migration'ları, `workflow/tables.py` ve API. SQL'in geçerli hali migration dosyalarıdır; aşağıdaki taslaktan küçük farkları vardır. Örneğin `not_verified`, kanıtı olmayan bir değer taşır. İkinci alt adım da uygulandı ([D38](../decisions.md)): `EvidenceCellDraft` v1 ve `TableColumnProposal` v1, `cell_extraction` ve `table_columns` adımları, doldurma, recheck ve sütun önerisi run'ları ile uçları. Buradaki taslaktan farklar: doldurma da tablonun `expected_version` değerini ister; doldurma planı (kaynaklar, sütunlar, hücre sürümleri) run'ın `target_json` alanında saklanır; recheck hedefi `cell_id` yerine sütun ve kaynak sürümüyle yazılır; verilen pasaj sayısı ve "bütün sayfalar verildi" bilgisi adım çıktısında değil StepInput'un `extraction_target.passage_scope` alanındadır; sınırı aşan kaynakta eşleşen pasajlardan sonra kalan yer sayfa sırasıyla doldurulur. Üçüncü alt adım (arayüz) de uygulandı: Evidence sekmesi ve hücre paneli (`apps/web/src/EvidenceTable.tsx`), fixture sunucusunda senaryolu modelle Playwright testi. §6'dan farklar: önerilen sütundaki "At" yalnız ekranda gizler, kayıt yazmaz; recheck'te yazılan pasaj sayısı istemcide hesaplanan bir üst sınırdır ("en fazla n pasaj"); tablo run'ları Answer sekmesindeki akışta değil, run kartında ve Activity'de görünür. Beşinci alt adım (gerçek model denemesi) iki kez yapıldı ([D43](../decisions.md)): `gpt-5.6-luna` ile canlı kütüphanenin kopyasında, 25 kaynak ve 8 sütunla. İlk denemede 23 model adımının 13'ü geçerliydi; aynı pasajdan birden çok alıntı artık kabul ediliyor (migration `0024`), yanlış pasaja verilen alıntıda onarım mesajı alıntının bulunduğu pasajı adlandırıyor ve IEEE indirme notu yeni çıkarımlardan ayıklanıyor. Tekrar denemede 19/23 adım geçerli oldu (yalnız özetli kaynaklar 14/14, PDF metinli 5/9); 12 hücrelik ajan örneklemesinde 10 doğru, 2 kısmen destekli, 0 yanlış. Bu bir insan doğrulaması değil ve tek araştırmayla sınırlı; parça sınırını aşan alıntılar hâlâ geçersiz kalıyor.
