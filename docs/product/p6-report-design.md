...
**Tarih:** 17 Eylül 2026. **Durum:** Kabul edildi (17 Eylül 2026): §2'deki kararlar ve §15'teki üç soru sahiple bu oturumda kapandı, §11'deki iki inceleme `gpt-5.6-sol` (effort high) ve `gpt-6-astra` (effort medium) ile yapıldı; ikincisinin talepleri ve sahibin üç ek kararı bu sürüme işlendi. Kod değişmedi. Bu not raporu (dilim 0, 1 ve 5) tanımlar; Chain of Ideas çizgileri (dilim 2), kill-search ve aday kartı (dilim 3) ile düzenleme ve ölçüm (dilim 4) ayrı notlarla gelir.
...
- **Dışa aktarma.** Dilim 1: Markdown (tablo Markdown tablosu, denklemler `$…$`, kaynakça numaralı). Dilim 5: IEEEtran LaTeX (`\cite{}` anahtarları `source_key`'den, tablo `table*`, denklemler değişmeden). İki çıktı da "DEIXIS ile üretildi; korpus: … sayıları" satırını II'den alır.
...
| Talep | Karar |
|---|---|
| 1 II/VIII kod çekirdeği ve aday kaynakçası dilim 1'e | Alındı (§12); LaTeX sonradan dilim 5 oldu |
...
0. **Eş zamanlı tablo doldurma (dilim 1'den önce, rapordan bağımsız).** `_table_fill` kaynak çağrılarını üst sınırlı eş zamanlı gönderir; kota hatasında sınır düşer; her kaynak adımı bugünkü gibi kendi kaydını yazar. Test: sahte adaptörle 12 kaynaklı doldurmada aynı anda açık çağrı sayısı sınırı aşmaz, bir kaynağın hatası diğerlerini durdurmaz, çıktı sıralı hâlle aynı. Ölçüm: D55'teki doldurma ile aynı kopyada süre karşılaştırması (506–576 s başlangıç). **Durum (2026-09-17, D61):** uygulandı, süre henüz ölçülmedi. `workflow/concurrency.py::ModelCallLimiter`, `Settings.model_concurrency` (varsayılan 6, `DEIXIS_MODEL_CONCURRENCY`). Açık sınır: Codex ve Claude Code adaptörleri tur boyunca iç kilit tuttuğu için bu bağlantılarda çağrılar hâlâ sırayla gider; beklenti dosyası: docs/product/p6-slice0-fill-expectations.md.
...
2. **Chain of Ideas: gelişim çizgileri ve alan tabanı.** Ayrı not (`p6-slice2-chain-of-ideas.md`). Bu bir erteleme konusu değildir: sahibin 14 Eylül 2026 kararı sentez yöntemini Chain of Ideas olarak seçti (README "Selected synthesis method", `docs/product/README.md` kabul edilen sınırlar) ve kabul edilen sıra şudur: gelişim çizgileriyle alan haritası → amaç, mekanizma ve değerlendirmeyle aday → her adayın belirli iddiasına kill-search. Bu notun önceki sürümü çizgileri ve alan tabanını dışarıda bırakmıştı; yanlıştı ve 17 Eylül 2026'da düzeltildi. Dilim şunları getirir: alan tabanı (kurucu çalışma, önemli derlemeler, yakın birincil çalışmalar; seçim atıf sayısının ötesinde gerekçelendirilir; `cited_by_count` tarihiyle zaten saklanıyor), kanıta bağlı gelişim çizgileri (her çalışma hangi problemi ele aldı, neyi kurdu ya da değiştirdi, sonraki hangi belirsizliğin peşine düştü; rakip dallar ve çapraz bağlar korunur; kronoloji ya da atıf tek başına bağ değildir, T11), rapora etkisi (III'te alanın gelişimi alt başlığı; VI'ya dördüncü aday türü: bir çizginin ucunda kalan ve korpusta peşine düşülmemiş belirsizlik; VII'de yönlerin çizgilerden türemesi) ve çizgi görünümü. Dilim 1 bunun için yer bırakır: `report_gaps.kind` kapalı liste değil genişleyebilir bir alandır ve III alt bölüm kabul eder.
3. **Kill-search ve aday kartı.** Ayrı not; adaylar dilim 2'nin çizgilerinden ve dilim 1'in türlerinden gelir, amaç–mekanizma–değerlendirme ile somutlaşır ve VI'daki adaylara durum verir.
...
5. **LaTeX.** IEEEtran dışa aktarımı, `source_key` tabanlı `\cite`, KaTeX ile IEEEtran arasında farklı davranan makroların denetimi. Dilim 1'den sonra herhangi bir anda yapılabilir; yönteme bağlı değildir.

Yöntem dayanağı: dilim 2 ve 3'ün notları README'deki altı adımlı Chain of Ideas uyarlamasından ve [research-methods.md](../methods/research-methods.md) §4'ten çıkar (aday kartı, iddia–pasaj matrisi, örtüşme/geçerlilik/değer ayrımı, elenen adayın sürümlenmesi); oradaki değerlendirme tablosundan R10'a iki ekilmiş hata daha alınır: aynı sonucun başka terminolojiyle zaten var olması ve atıf var diye gelişim ilişkisi kurulması.
