...
9. **Test:** önce başarısız test. Dilim, `PYTHONPATH=backend uv run pytest` tamamen yeşilken ve arayüze dokunduysa `npm run build && npm run lint` ile Playwright A–G geçerken biter. Geçen test iş akışı davranışını gösterir, model kalitesini değil.
...
Kabul: ifade listesi boşsa ölçüt pasajları konu düzenine düşer; alıntı doğrulaması sayfa metninde koşmaya devam eder. Ölçülmeyen: iki kotanın 48 pasajlık çok kaynaklı girdideki etkisi; dilim bunu ölçer.
