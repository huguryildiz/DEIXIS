...
`s2share.py` her sağlayıcının saklanan sayfalarını okudu. Semantic Scholar'ın getirdiği doğrulanmış eserlerin neredeyse hepsini başka bir sağlayıcı da getirdi: yalnız onun getirdiği 3 eser vardı (`quick` g009, `detailed` g030 ve g126) ve hiçbiri özet aşamasını geçmedi. Buna karşılık `detailed`'da aramanın süresini o belirledi: 24 sayfa adımı, 44 istek, sayfa başına ortanca ~19 sn (öbür sağlayıcılarda 2–7 sn). Sağlayıcı başına 300 kayıtlık bir okuma sınırı önerildi; sahip onun yerine Semantic Scholar'ın Scopus gibi (D91) sw aramasından çıkıp yalnız özet tamamlama kaynağı olarak kalmasına karar verdi. Kod değişmedi; kararı ve kaydını (D93) ayrı bir dilim yazar, kabul koşulu paket sorusunda da kayıp olmadığını göstermektir.
