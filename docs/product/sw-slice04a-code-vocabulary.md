...
- `detect_language`: `language_hint` varsa o belirler (`en` ile başlıyorsa `en`, değilse `other`). Yoksa soru sözcüklerinin `ENGLISH_FUNCTION_WORDS` içindeki payı eşiğin üstündeyse `en`. Latin dışı harf içeren soru `other`'dır.
