...
- **Sahibe soru:** OpenAlex her kayda `keywords` verir ama bunlar yazarın değil, OpenAlex'in konu modelinin atadığı sözcüklerdir. SW2.4 "yazar anahtar sözcükleri" der, bu yüzden alınmadı. Bedeli: IEEE ya da PubMed anahtarı olmayan kullanıcıda (SW8'in tarif ettiği kullanıcı) genişleme yalnızca başlık n-gramlarıyla çalışır. Ayrı köken etiketiyle (`provider_keyword`) almak küçük bir ek dilimdir.
