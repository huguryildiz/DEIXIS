...
- **Karar (21 Eylül 2026, sahip plan sohbetine bıraktı): OpenAlex'in `keywords` alanı alınmaz.** Bunlar yazarın değil, OpenAlex'in konu modelinin atadığı, denetimli bir listeden gelen genel sözcüklerdir; SW2.4 "yazar anahtar sözcükleri" der. Ayrıca alanı istemek OpenAlex'in `select` parametresini, yani `legacy` isteğini ve saklı yük özetlerini değiştirirdi. Bedeli: IEEE ya da PubMed kaydı gelmeyen araştırmada genişleme yalnızca başlık n-gramlarıyla çalışır. Dilim 24 başlık n-gramlarının zayıf kaldığını gösterirse ayrı köken etiketiyle (`provider_keyword`) ve yalnızca `sw` isteğinde alınması yeniden açılır.
