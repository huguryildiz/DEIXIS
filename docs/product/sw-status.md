...
| 04a | Kodla sözcük dağarcığı ve kavram blokları (çıkarma, sayım sınaması, blok sorgusu, `sw` keşfine bağlama) | Kur (ölçülmedi) | Opus · high | toplu | dosya hazır | [dilim](sw-slice04a-code-vocabulary.md) · [prompt](sw-slice04a-prompt.md) | | Plandaki dilim 04 ikiye bölündü. **Sahibe soru:** `sw` tek geniş sorgu atıyor ama sorgu başına en çok 25 kayıt okunuyor; sayfalama ve `sw` okuma bütçesi hiçbir dilimde yok, dilim 07'den önce bir yere yazılmalı. |
