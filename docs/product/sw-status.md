...
| 04c | `sw` aramasında sayfalama ve okuma bütçesi | Kur | Opus · medium | toplu | dosya hazır | [dilim](sw-slice04c-paging-and-read-budget.md) · [prompt](sw-slice04c-prompt.md) | | Önkoşul 04a; dilim 07'nin önkoşulu. Tarama kesimi (`max_candidates`) dilim 09'a kadar durur. Bugün hiçbir sağlayıcı sayfalamıyor; dilim dokuzuna da `cursor` ekler (OpenAlex imleç, diğerleri ofset, SerpApi tek sayfa, Semantic Scholar 1.000'de durur). Okuma sınırı `SW_READ_LIMIT = 2.000`, elle seçildi. 04d ile aynı anda koşabilir: `flow.py`'de yalnızca arama döngüsü, `_search` ve yeni `_search_pages`. Yorum: başarısız sayfa o sorgunun okumasını o koşuda bitirir, öbür sorguları durdurmaz. |
| 04d | Blok atamasını modele sordur, kararı kodda tut (SW17) | Kur | Opus · high | tam | uygulanıyor | [dilim](sw-slice04d-model-block-labelling.md) · [prompt](sw-slice04d-prompt.md) | | Önkoşul 04a; 04b ve 04c'den bağımsız. Ölçüldü: `.local/sw-block-labelling-2026-09-21/` — 8 soru, 28 ifade, kural 19/28, `deepseek-flash` 28/28, üç koşuda tam kararlı, çağrı başına ~2,3 sn / ~870 jeton. Kuralın edat vetosu bilerek kaldırıldı (öneriye sapma: veto düzeltilen iki vakayı da geri bozuyordu). `skill_package_hash` bu dilimde değişir. Ölçülmeyenler: tek gerçek iddia vakası, tek koşunun yeterliliği, İngilizce olmayan ve bozuk sorular, model hatası yolu. |
